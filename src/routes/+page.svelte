<script>
	...
	import { expand, inner, outer } from './custom.js';
	...
	$: {
		if (checked) {
			setTimeout(() => (rotatedClass = '-scale-x-[100%]'), 700);
		}
	}
	onMount(() => {
		checked = true;
...
<div class="container h-full mx-auto flex justify-center pt-10">
	<div class="gap-10 text-center flex flex-col items-center">
		<div class="flex font-semibold">
			<h3 class="h3">Welcome to WHYNOT</h3>
			<h3 class="h3 text-secondary-500">PC</h3>
		</div>
		<h4 class="h4">
			Built with
			<a
				...
				target="_blank"
				class="text-tertiary-400 font-semibold">SvelteKit</a
			>
			&
			<a
				...
				rel="noreferrer nofollow"
				target="_blank"
				class="text-tertiary-400 font-semibold">SkeletonUI</a
			>
		</h4>
		<!-- Animated Logo -->
		<div class="flex justify-center relative">
			<section class="img-bg w-64 h-64 flex-justify-center" />

			<div class="flex gap-x-2 sm:gap-x-10 md:gap-x-20">
				<figure class="w-[150px] sm:w-[190px]">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103 124">
						<g opacity="0.2">
							<path in:fade={{ duration: 500 }} style="stroke:none" d={inner} />

							{#if checked}
								<path
									in:expand={{ duration: 400, delay: 1200, easing: quintOut }}
				...
				<figure class="w-[160px] sm:w-[200px]">
					<svg
						class="fill-token {rotatedClass}"
		...
		<div class="flex justify-center my-auto">
			<a class="card p-2 variant-ghost-primary h4" href="/catalog">Go to catalog</a>
		</div>
		<div class="mt-auto mb-[1rem]">
			<p
				style="font-family: monospace"
				class="p-1.5 font-serif rounded border border-tertiary-800 h6"
			>
				© whynotPC 2023, All Rights Reserved
			</p>
		</div>
	</div>
</div>
...
<style lang="postcss">
	.img-bg {
		...
		animation:
			pulse 5s cubic-bezier(0, 0, 0, 0.3) infinite,
